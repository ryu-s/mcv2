...
namespace OpenrecSitePluginTests
{
    ...
    class CommentProviderTests
    {
        ...
        {
            var guid = new SitePluginId(Guid.NewGuid());
            ...
            var cpMock = new Mock<CommentProvider2>(siteOptions, logger);
            cpMock.Protected().Setup<List<Cookie>>("GetCookies", ItExpr.IsAny<IBrowserProfile2>()).Returns(new List<Cookie>
            {
            ...
            cp.MessageReceived += (s, e) =>
            {
                if (e.Message is IOpenrecComment comment)
                {
                    ...
                    Assert.AreEqual("16:13:06", comment.PostTime.ToString("HH:mm:ss"));
            ...
            var f = await Task.WhenAny(t, timeoutT);
            if (f == timeoutT)
            {
                Assert.Fail("timeout");
            }
